...
## It also generates a report that includes the URL of the question made by topic,
## the output file is then treated in Excel for calculating totals 
...
############################### Comment the following lines  to run in PC ##############################
args <- commandArgs(trailingOnly = TRUE)
outDir <- args[1]
readFrom <- paste(args[1], args[2],sep="")
year  <-  as.integer(args[3])

################################# Uncomment the following lines to run in PC ###########################
## readFrom <- "db2.xml.csv"
## year <- 2015
########################################################################################################
...
## corp <- createCorp(readFrom) ## replaced by:
...
saveTo <- paste(args[1],"report-", args[2], "-", year, ".txt", sep = "")
#################################### Uncomment for running in PC ####################################
##saveTo <- paste("report-", readFrom, "-", year, ".txt")
#####################################################################################################

################################### Comment for running in PC ########################################################
write.csv(topic.keyword, file = paste(outDir,"lda_keywords_", args[2],"_",year, "-", topicCount, ".csv", sep = "") )
################################### Uncomment for running in PC ######################################################
## write.csv(topic.keyword, file = paste("TEST_", readFrom, "_", year, "-", topicCount, ".csv", sep = "") )
...
t <- topics(mdl)
l <- length(topics(mdl))
saveTo <- paste(args[1], "report-", args[2], "-", year, ".txt") ## path to write the report file
