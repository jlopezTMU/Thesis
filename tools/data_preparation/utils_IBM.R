...
## This function returns topic keywords for a given LDA model
## dat - Document Term Matrix to feed to the LDA model
## topicCount - Number of topics in the LDA model
## JL changed name of this function to getTopicsKeyword 
getTopicsKeyword <- function(dat, topicCount){
  mdl <- LDA(dat, topicCounthelp) #LDA model
  
  topic.keyword <- terms(mdl[[VEM]],topicCount)
  ...
  ## mdl.beta.sd <- sd(mdl@beta)
    ...
    ##topic.keyword = as.vector(table(topics( mdl )))
  ## ))
}
