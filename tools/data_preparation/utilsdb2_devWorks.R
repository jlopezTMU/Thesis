...
createCorp <- function(readFromfileName){
  ...
  library(tm)
  ## Load the data
  Posts <- read.delim(file = readFromfileName, header = F, quote = "", sep = "\t")
  
  cat("Read", nrow(Posts), "rows from", readFromfileName, "\n")
  
  ## These lines useless for processing devWorks
  ## the following lines tweaked to filter by year
  ##if(! missing(year)){
  ##  Posts$create_ts <- as.POSIXlt(Posts$create_ts)
  ##  Posts <- subset(Posts, Posts$create_ts$year == (year - 1900))
    ## DONT NEED THIS will work with Posts code added to support reporting
    ## Qid <- data.frame(i = Posts$id, q = Posts$title) ## this to save id-original question
    ##
    cat("Kept", nrow(Posts), "rows\n")
  ## }
...
doCorpCreation <- function(Posts){
  ...
  ## Posts <- data.frame(doc_id = Posts$id, txt = paste(Posts$title, Posts$body, Posts$answers, sep =" ")) ## commented out
  ...
  ##posts.reader <- readTabular(mapping=list(content="txt", id="doc_id")) ## commented out 
  ## what is this?????
