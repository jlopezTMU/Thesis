...
data_filter_200 <- FALSE


readFromfileName = "topX.zip"

if(data_filter_200){
  model_file_name <- "./models_fitting.remove_top_25_percent_and_values_gt_200.rda"
  models_performance_file_name <- "./models_performance.remove_top_25_percent_and_values_gt_200.csv"
  filter_name <- "DS2"
}else{
  model_file_name <- "./models_fitting.remove_top_25_percent.rda"
  models_performance_file_name <- "./models_performance.remove_top_25_percent.csv"
  filter_name <- "DS1"
}
...
dat <- read.csv(unz(readFromfileName, "topX.csv"), header = T, sep = ",", row.names = NULL)
...
for(iG in 1:length(validGroups)) {
  ...
  if(data_filter_200)
    pdf(paste(paste("./figures/DS2/fit_model_one_param_complex_", p3, p1, p2, filter_name, sep = "_"), ".pdf", sep = ""))
  else
    pdf(paste(paste("./figures/DS1/fit_model_one_param_complex_", p3, p1, p2, filter_name, sep = "_"), ".pdf", sep = ""))
  
  line_counter <- 1
  topX_values_to_plot <- c(5,10,25,50)
  ...
  legend("bottomleft", 
         ...
         col = c(colour[1], colour[2], colour[3], colour[4]), lty = 1)
  dev.off()
